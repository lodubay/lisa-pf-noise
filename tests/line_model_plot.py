...
runs = ['ltp_run_b', 'ltp_run_c', 'drs_run_q', 'drsCrutch_run_q', 'run_k']
channels = range(6)
...
for run in runs:
    ...
    for channel in channels:
        ...
        if os.path.exists(counts_file):
            ...
            sys.stdout.write('\n')
            sys.stdout.flush()
            counts = pd.DataFrame(counts, index=gps_times).astype(float)
        ...
        plt.close()
        print(os.path.join(output_dir, 'plots', 'linecounts' + str(channel) + '.png'))
        #plt.show()
